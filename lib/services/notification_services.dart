...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    var formatteDate = DateFormat.yMd().parse(date);
    final tz.TZDateTime formatteLocalDate =
        tz.TZDateTime.from(formatteDate, tz.local);
    tz.TZDateTime scheduledDate = tz.TZDateTime(
        tz.local,
        formatteLocalDate.year,
        formatteLocalDate.month,
        formatteLocalDate.day,
        hour,
        minutes);
    ...
    if (scheduledDate.isBefore(now)) {
      if (repeat == 'Daily') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (formatteDate.day) + 1, hour, minutes);
      }
      if (repeat == 'Weekly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (formatteDate.day) + 7, hour, minutes);
      }
      if (repeat == 'Monthly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year,
            (formatteDate.month) + 1, formatteDate.day, hour, minutes);
      }
      scheduledDate = afterRemind(remind, scheduledDate);
    }
